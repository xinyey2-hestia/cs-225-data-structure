                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty()){
     ...
     if ((single->freq).getFrequency() > (merge->freq).getFrequency()){//merge smaller
       smallest = merge;
       mergeQueue.pop();
     }
     else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while((singleQueue.size()+mergeQueue.size())>1){
       ...
       TreeNode* inner = new TreeNode(first->freq.getFrequency()+second->freq.getFrequency());
       inner->left = first;
       inner->right = second;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     while (bfile.hasBits()){
     if (bfile.getNextBit()){   // case 1
       TreeNode* result = new TreeNode(Frequency(bfile.getNextByte(),0));
       return result;
     }
     else{
